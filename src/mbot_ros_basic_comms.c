#include "mbot_ros_basic_comms.h"
#include "mbot_basic_ros.h" // For mbot_state_t, mbot_cmd_t, and config defines
...
geometry_msgs__msg__Vector3Stamped encoder_msg;
...
int mbot_ros_comms_init_messages(rcl_allocator_t* allocator) {
    ...
    memset(&encoder_msg, 0, sizeof(encoder_msg)); // geometry_msgs/Vector3: x=left (MOT_L), y=right (MOT_R), z=unused (MOT_UNUSED)
...
int mbot_ros_comms_init_publishers(rcl_node_t *node) {
    ...
    ret = rclc_publisher_init_best_effort(
        ...
        "encoders");
    if (ret != RCL_RET_OK) { printf("[FATAL] Failed to init encoder_publisher: %d\n", ret); fflush(stdout); return MBOT_ERROR; }
