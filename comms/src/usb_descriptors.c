...
#define EPNUM_CDC_1_OUT     0x04 // out endpoint for CDC 1
...
uint8_t const desc_configuration[] = {
    ...
    // CDC 0: Communication Interface - Get size from tusb_config.h
    TUD_CDC_DESCRIPTOR(ITF_NUM_CDC_0, 4, EPNUM_CDC_0_NOTIF, 8, EPNUM_CDC_0_OUT, EPNUM_CDC_0_IN, CFG_TUD_CDC_EP_BUFSIZE),

    // CDC 1: Communication Interface - Get size from tusb_config.h
    TUD_CDC_DESCRIPTOR(ITF_NUM_CDC_1, 5, EPNUM_CDC_1_NOTIF, 8, EPNUM_CDC_1_OUT, EPNUM_CDC_1_IN, CFG_TUD_CDC_EP_BUFSIZE),
};
...
char const *string_desc_arr[] = {
    ...
    "MBot Debug Console",           // 4: CDC Interface 0
    "MBot MicroROS Data",           // 5: CDC Interface 1
...
uint16_t const *tud_descriptor_string_cb(uint8_t index, uint16_t langid)
{
    ...
    switch (index) {
        ...
        case STRID_SERIAL: {
            // Use Pico SDK to get unique board ID as serial
            char serial_str[2 * PICO_UNIQUE_BOARD_ID_SIZE_BYTES + 1];
            pico_get_unique_board_id_string(serial_str, sizeof(serial_str));
            char_count = strlen(serial_str);
